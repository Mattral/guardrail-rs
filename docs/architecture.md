...
Requests flow through a single in-process **pipeline** of **stages** before
being forwarded to an upstream LLM provider.
...
```text
  ...
  Client ── HTTP ──────▶│ 1. Read & size-limit body                     │
                        ...
                        │ 4a. Block  → JSON 403, audit log, return      │
...
- [`forward`](../crates/guardrail-proxy/src/forward.rs) — sends the
  ...
  forwarding the original `Authorization` header.
- [`metrics`](../crates/guardrail-proxy/src/metrics.rs) — Prometheus
  ...
  redaction counts, and latency histograms (pipeline-level and per-stage).
...
`guardrail-proxy`: `run`, `validate`, `check`.
...
`ConfigHandle` holds two `ArcSwap`s: one for `Config`, one for `Pipeline`.
`ConfigHandle::reload()`:

1. Re-reads and re-validates the TOML file (errors leave the existing config
   active).
2. Rebuilds the entire `Pipeline` (recompiling regexes, reloading ONNX
   sessions if paths changed).
3. Atomically swaps both `ArcSwap`s.

In-flight requests that already called `ConfigHandle::pipeline()` continue
using their snapshot `Arc<Pipeline>` to completion; only requests that arrive
*after* the swap see the new configuration. There is no lock contention on
